...
class PatchBattle extends Battle {
  getTeam(team) {
    if (team) {
      return team;
    }
...
class TestTools {
   ...
   * @returns {PatchBattle} A battle.
   */
  createBattle(options, teams, prng) {
    if (Array.isArray(options)) {
      teams = options;
      options = {};
    }
    const format = this.getFormat(options || {});
    const battle = new PatchBattle(format.id, undefined, undefined, prng);
...
const common = module.exports = new TestTools();
